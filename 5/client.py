...
com1=["register","login","delete","logout"]
com2=["post","receive-post","invite","list-invite",
      "accept-invite","list-friend","send-group",
...
while True:
  ...
  if arg_list[0] in com1:
    ...
    if arg_list[0] == "register":
      ...
      if y["status"]==0:
        ...
        ip_dict[arg_list[1]] = y["ip"]
        con_dict[y["token"]] = stomp.Connection10([('54.249.144.111',61613)])
  ...
  elif arg_list[0] in com2: 
    if len(arg_list)>1:
      if arg_list[1] in id_list:
        id_tmp = arg_list[1]
        host2  = ip_dict[arg_list[1]]
        arg_list[1] = id_list[arg_list[1]] 
        arg = " ".join(arg_list)
        client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)   
        client.connect((host2, 8888))
        client.send(arg)
        response = client.recv(4096)
        y = json.loads(response)
        #print(response)    
